...
st.set_page_config(
    page_title="Nano Sage: The Lightweight Knowledge Agent",
    page_icon=":zap:",
    layout="wide",
)
st.markdown(CUSTOM_CSS, unsafe_allow_html=True)
agent_name = "nano_sage"


async def header():
    st.markdown("<h1 class='heading'>Nano Sage</h1>", unsafe_allow_html=True)
    st.markdown(
        "<p class='subheading'>A lightweight but powerful AI assistant powered by ChatGPT 4.1 nano. Fast, efficient, and cost-effective.</p>",
...
async def body() -> None:
    ...
    if (
        ...
        logger.info("---*--- Creating Nano Sage Agent ---*---")
        nano_sage = get_nano_sage(user_id=user_id, model_id=model_id)
        st.session_state[agent_name]["agent"] = nano_sage
        st.session_state["selected_model"] = model_id
    else:
        nano_sage = st.session_state[agent_name]["agent"]

    ####################################################################
    # Load Agent Session from the database
    ####################################################################
    try:
        st.session_state[agent_name]["session_id"] = nano_sage.load_session()
    except Exception:
    ...
    ####################################################################
    if nano_sage.memory:
        agent_runs = nano_sage.memory.runs
    ...
    # Get user input
    ####################################################################
    if prompt := st.chat_input("⚡ How can I help you quickly?"):
    ...
    if last_message and last_message.get("role") == "user":
        ...
        with st.chat_message("assistant"):
            ...
            with st.spinner(":zap: Processing quickly..."):
                response = ""
                try:
                    # Run the agent and stream the response
                    run_response = await nano_sage.arun(user_message, stream=True)
                    ...
                    if nano_sage.run_response is not None:
                        await add_message(agent_name, "assistant", response, nano_sage.run_response.tools)
                    else:
    ...
    # Knowledge widget
    ####################################################################
    await knowledge_widget(agent_name, nano_sage)

    ####################################################################
    # Session selector
    ####################################################################
    await session_selector(agent_name, nano_sage, get_nano_sage, user_id, model_id)

    ####################################################################
    # About section
    ####################################################################
    await utilities_widget(agent_name, nano_sage)
