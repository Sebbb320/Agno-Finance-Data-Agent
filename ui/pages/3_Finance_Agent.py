...
st.set_page_config(
    page_title="Finance Agent: 金融数据分析与可视化",
...
async def header():
    st.markdown("<h1 class='heading'>Finance Agent</h1>", unsafe_allow_html=True)
    st.markdown(
        "<p class='subheading'>专注于金融数据分析、实时信息检索、Pandas数据处理和Altair可视化的智能体。</p>",
...
async def body() -> None:
    ...
    if (
        ...
        finance_agent = get_finance_agent(user_id=user_id, model_id=model_id)
        st.session_state[agent_name]["agent"] = finance_agent
        st.session_state["selected_model"] = model_id
    else:
        finance_agent = st.session_state[agent_name]["agent"]

    ####################################################################
    # Load Agent Session from the database
    ####################################################################
    try:
        st.session_state[agent_name]["session_id"] = finance_agent.load_session()
    except Exception:
    ...
    ####################################################################
    if finance_agent.memory:
        agent_runs = finance_agent.memory.runs
    ...
    # Get user input
    ####################################################################
    if prompt := st.chat_input("💹 请输入你的金融分析需求..."):
    ...
    if last_message and last_message.get("role") == "user":
        ...
        with st.chat_message("assistant"):
            ...
            with st.spinner(":chart_with_upwards_trend: 正在分析金融数据..."):
                response = ""
                try:
                    # Run the agent and stream the response
                    run_response = await finance_agent.arun(user_message, stream=True)
                    ...
                    if finance_agent.run_response is not None:
                        await add_message(agent_name, "assistant", response, finance_agent.run_response.tools)
                    else:
    ...
    ####################################################################
    # Knowledge widget (File upload) - 暂时禁用
    ####################################################################
    # await knowledge_widget(agent_name, finance_agent)

    ####################################################################
    # Session selector
    ####################################################################
    await session_selector(agent_name, finance_agent, get_finance_agent, user_id, model_id)

    ####################################################################
    # About section
    ####################################################################
    await utilities_widget(agent_name, finance_agent)
