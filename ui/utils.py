...
async def selected_model() -> str:
    ...
    model_options = {
        ...
        "gpt-4.1-nano": "gpt-4o-mini",  # ChatGPT 4.1 nano (using gpt-4o-mini as equivalent)
        "o3-mini": "o3-mini",
    }
...
async def example_inputs(agent_name: str) -> None:
    ...
    with st.sidebar:
        ...
        elif agent_name == "nano_sage":
            if st.button("Quick summary of AI trends"):
                await add_message(
                    agent_name,
                    "user",
                    "Give me a quick summary of current AI trends",
                )
            if st.button("Fast coding help"):
                await add_message(
                    agent_name,
                    "user",
                    "Help me write a simple Python function to sort a list",
                )
        elif agent_name == "finance_agent":
            if st.button("分析AAPL股票"):
                await add_message(
                    agent_name,
                    "user",
                    "分析AAPL近30天的股票走势，并提供可视化图表",
                )
            if st.button("获取市场新闻"):
                await add_message(
                    agent_name,
